...
def load_data(file_path:str)->pd.DataFrame:
  ...
  try:
    ...
    logger.debug('Data loaded add NaNs filled from %s',file_path)
    return df
  except pd.errors.ParserError as e:
    logger.error("Failed to aprse the csv file:%s",e)
    raise 
  except Exception as e:
...
def apply_tfidf(train_data:pd.DataFrame,max_features:int,ngram_range:tuple)->tuple:
  try:
    ...
    logger.error("Error duringTF-IDF transformation:%s",e)
    raise 
...
def train_lgbm(X_train: np.ndarray, y_train: np.ndarray, learning_rate: float, max_depth: int, n_estimators: int) -> lgb.LGBMClassifier:
    """Train a LightGBM classifier and return the trained model."""
    try:
